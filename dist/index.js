...
var UserService = class {
  ...
  async findAll() {
    try {
      ...
      if (!users2 || users2.length === 0) {
        return ServiceResponse.failure("No Users found", null, import_http_status_codes4.StatusCodes.NOT_FOUND);
      }
  ...
  async findById(id) {
    try {
      ...
      if (!user) {
        return ServiceResponse.failure("User not found", null, import_http_status_codes4.StatusCodes.NOT_FOUND);
      }
...
var customLogLevel = (_req, res, err) => {
  if (err || res.statusCode >= import_http_status_codes6.StatusCodes.INTERNAL_SERVER_ERROR) return "error" /* Error */;
  if (res.statusCode >= import_http_status_codes6.StatusCodes.BAD_REQUEST) return "warn" /* Warn */;
  if (res.statusCode >= import_http_status_codes6.StatusCodes.MULTIPLE_CHOICES) return "silent" /* Silent */;
  return "info" /* Info */;
};
var customSuccessMessage = (req, res) => {
  if (res.statusCode === import_http_status_codes6.StatusCodes.NOT_FOUND) return (0, import_http_status_codes6.getReasonPhrase)(import_http_status_codes6.StatusCodes.NOT_FOUND);
